...
from fresher.n02_operator_expression import *
...
def test_question_10():
    qname = 'question 10'
    def test_func(res_a, answer_a, res_b, answer_b, res_c, answer_c):
        assert res_a == answer_a, ASSERT_FAILED.format(qname)
        assert res_b == answer_b, ASSERT_FAILED.format(qname)
        assert res_c == answer_c, ASSERT_FAILED.format(qname)
    question_10([test_func])
    logger.info(f'{qname} passed')
